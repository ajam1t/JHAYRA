                ...
                <div key={id} className="cart-item">
                  <div className="cart-item-img" style={{overflow:'hidden'}}>
                    {art ? (
                      ...
                      <div style={{width:'100%',height:'100%',background:'var(--bg)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'.65rem',color:'var(--muted)',textAlign:'center',padding:'.3rem'}}>{product.name}</div>
                    )}
                  </div>
                  <div>
                    <div className="cart-item-name">{product.name}</div>
                    <div className="cart-item-meta">{product.category}</div>
                    <div className="qty">
                  ...
                  <div style={{fontWeight:700,fontFamily:'var(--fd)'}}>₹{(product.price*qty).toLocaleString('en-IN')}</div>
                </div>
              );})
            )}
          </div>
